    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0., out_indices = (9, 14, 19, 23), patch_w_num = 32):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, WindowAttention2(dim, heads = heads, dim_head = dim_head, dropout = dropout, patch_w_num=patch_w_num)),
...
class ViTRoad(nn.Module):
    def __init__(self, num_classes=1, num_channels=3):
        ...
        self.neckblock = Transformer(dim = 512, depth = 4, heads = 8, dim_head = 128, mlp_dim = 1024, out_indices = -1, patch_w_num = 32)
    ...
    def forward(self, x):
        ...
        # e4 = self.dblock(e4)
        b, c, h, w = e4.shape
        e4 = rearrange(e4, 'b c h w->b (h w) c')
        e4 = self.neckblock(e4)[0]
        e4 = rearrange(e4, 'b (h w) c->b c h w', h = h)
